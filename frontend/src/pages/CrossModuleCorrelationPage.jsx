...
// Calls POST /api/ai/cross-module-correlation. Surfaces 503 if the backend
// reports the OPENROUTER_API_KEY is missing.
export default function CrossModuleCorrelationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/ai/cross-module-correlation', { department });
      setResult(res.data);
    } catch (err) {
    ...
    <div className="feature-page">
      <div className="feature-page-header">
        <div className="back-section">
          ...
          <h1>🔗 Cross-Module Correlation</h1>
        </div>
      </div>
      <div className="data-table-container" style={{ padding: '24px' }}>
        <p style={{ color: '#94a3b8', marginBottom: '20px' }}>
          Cross-correlate retention, engagement, pulse, and exit-interview signals for a department to surface composite risk patterns.
        </p>
        <form onSubmit={handleSubmit}>
          ...
          <div className="modal-actions">
            <button type="submit" className="btn-save" disabled={loading}>{loading ? 'Analyzing...' : '🤖 Run Correlation'}</button>
          </div>
        </form>
        {error && (
        ...
        {result && (
          <div style={{ marginTop: '20px' }}>
            ...
            <AIAnalysisDisplay data={result.correlation_analysis} />
          </div>
        )}
      </div>
    </div>
